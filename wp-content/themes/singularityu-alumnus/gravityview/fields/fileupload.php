...
if( !empty( $value ) ){
	...
	// There are multiple files
		...
		if( !empty( $output ) ) {
            ...
            $output = sprintf("<ul class='gv-field-file-uploads %s'>%s</ul>", $gv_class, get_avatar(get_current_user_id()) );
        }
	}
    ...
    $get_curr_entry = $gravityview_view->getCurrentEntry();
    $userID =  $get_curr_entry['created_by'];
    $output = get_avatar($userID,512);
